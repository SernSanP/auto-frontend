...
import { JSXElementConstructor, ReactElement, ReactNodeArray, ReactPortal, useCallback, useState } from 'react';
...
export default function Papaparse() {
  ...
  async function onUpload() {
    ...
    const source_system_name = localStorage.getItem('source_system_name')
    await axios.post('http://localhost:5000/transfer/create', {
      data: result.res,
      userID,
      source_system_name,
    });
  ...
  const showerror = (data: any) => {
    return (
      <div>
        ...
        <div className="pl-6">
          {data.errors.map((error: string | number | boolean | {} | ReactElement<any, string | JSXElementConstructor<any>> | ReactNodeArray | ReactPortal | null | undefined) => (
            <div>- {error}</div>
          ))}
        </div>
      </div>
          ...
          {result.error ? (
            <div>
              <div className="flex items-center justify-center gap-2 pt-4">
                <img
                  src="https://image.freepik.com/free-icon/forbidden-simbol_318-9698.jpg"
                  className="w-20 h-20"
                />
              </div>
              ...
              <div className="pt-4 pb-4 text-center">
                โปรดตรวจสอบไฟล์ CSV ของท่านดังต่อไปนี้
              </div>
            ...
            <div>
              <div className="flex items-center justify-center gap-2 pt-4">
                <img
                  src="https://image.flaticon.com/icons/png/512/20/20406.png"
                  className="w-20 h-20"
                />
              </div>
              <div className="pt-4 text-lg font-bold">Upload Confirm</div>
              <div className="grid grid-cols-4 pt-4 gap-4">
                <div className="col-span-2">
                  Total {result.res.length} Transactions
                </div>
